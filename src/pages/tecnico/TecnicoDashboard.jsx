...
export default function TecnicoDashboard() {
  ...
  const agregarRepuesto = (repuestoId, cantidad) => {
    ...
    setRepuestosTemp(prev => {
      ...
      if (ex) return prev.map(r => r.repuestoId === id ? { ...r, cantidad: r.cantidad + cnt } : r);
          ...
          <div className="page-section active">
            ...
            <div className="page-content">
              <div>
                ...
                <div className="card">
                  <div className="card-header"><h3>Mis Órdenes de Servicio</h3></div>
                  <div className="table-wrap">
                    <TablaOrdenesTecnico servicios={misServs} clientes={clientes} repuestos={repuestos} onAbrir={abrirModal} />
                  </div>
                </div>
              </div>
                    ...
                    <div key={sv.id} className="card" style={{ marginBottom: 16 }}>
                      <div className="card-header">
                        <div>
                          ...
                          <p style={{ fontSize: 13, color: '#666', margin: 0 }}>{formatearFecha(sv.fecha)} {sv.hora}</p>
                        </div>
                        <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
                          <EstadoBadge estado={sv.estado} />
                          <button className="btn btn-primary btn-sm" onClick={() => abrirModal(sv)}>Actualizar</button>
                        </div>
                      </div>
                      <div className="card-body" style={{ padding: '12px 16px', fontSize: 14 }}>
                        ...
                        <p><strong>Total:</strong> <span className="text-blue text-bold">{formatearPeso(totalServicio(sv, repuestos))}</span></p>
                      </div>
                    </div>
                  );
                })
      ...
      {modalActualizar && servActual && (
        <Modal titulo="Actualizar Orden de Servicio" onClose={() => setModalActualizar(false)}
          ...
          <div className="form-group">
            ...
            <textarea value={updNotas} onChange={e => setUpdNotas(e.target.value)} placeholder="Observaciones, trabajo realizado..." />
          </div>
              ...
              return (
                <div key={i} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', fontSize: 13, padding: '4px 0', borderBottom: '1px solid #eee' }}>
                  ...
                  <span style={{ display: 'flex', gap: 8, alignItems: 'center' }}>
                    <span className="text-blue">{formatearPeso((rep?.precio || 0) * r.cantidad)}</span>
                    <button className="btn btn-secondary btn-sm" onClick={() => quitarRepuesto(i)}>✕</button>
                  </span>
                </div>
              );
            })}
...
function TablaOrdenesTecnico({ servicios, clientes, repuestos, onAbrir }) {
  return (
    <table>
      <thead><tr><th>#</th><th>Cliente</th><th>Tipo</th><th>Fecha</th><th>Estado</th><th>Total</th><th>Acción</th></tr></thead>
      <tbody>
        {servicios.length === 0
          ...
          : servicios.map(sv => {
              ...
              return (
                <tr key={sv.id}>
                  ...
                  <td><button className="btn btn-primary btn-sm" onClick={() => onAbrir(sv)}>Actualizar</button></td>
                </tr>
              );
            })
        }
    ...
    <div className="form-row" style={{ alignItems: 'flex-end', marginTop: 10 }}>
      <div className="form-group" style={{ marginBottom: 0 }}>
        ...
        <select value={sel} onChange={e => setSel(e.target.value)}>
          ...
          {repuestos.map(r => <option key={r.id} value={r.id}>{r.icono} {r.nombre} — {formatearPeso(r.precio)}</option>)}
        </select>
      </div>
      <div className="form-group" style={{ marginBottom: 0 }}>
        <label>Cantidad</label>
        <input type="number" value={cnt} onChange={e => setCnt(e.target.value)} min="1" max="99" style={{ width: 70 }} />
      </div>
      <button className="btn btn-secondary btn-sm" style={{ marginBottom: 0 }}
        onClick={() => { onAgregar(sel, cnt); setSel(''); setCnt('1'); }}>
        + Agregar
      </button>
    </div>
  );
