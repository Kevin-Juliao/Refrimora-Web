...
export default function AdminDashboard() {
  ...
  const kpis = [
    ...
    {
      label: 'En curso',
      valor: servicios.filter(s => !['finalizado', 'cancelado'].includes(s.estado)).length,
      icon: '🔧',
      cls: 'orange'
    },
    {
    ...
    },
    {
      ...
      valor: tecnicos.filter(t => t.disponible).length,
      icon: '🧑‍🔧',
      cls: 'green'
    },
  ];
    ...
    <div className="ad-shell">
            ...
            <div className="ad-hero">
              ...
              {sols.length > 0 && (
                <button className="ad-hero-notif" onClick={() => setSeccion('solicitudes')}>
                  ...
                  📨 {sols.length} solicitud{sols.length !== 1 ? 'es' : ''} web pendiente
                  {sols.length !== 1 ? 's' : ''}
                </button>
              )}
            </div>
            ...
            <div className="ad-grid-main">
              <div className="ad-panel">
                <div className="ad-panel-header">
                  ...
                  <button className="ad-btn-xs" onClick={() => setSeccion('servicios')}>
                    Ver todas →
                  </button>
                </div>
                  ...
                  <tbody>
                    {clientes.map(c => (
                      <tr key={c.id}>
                        ...
                        <td className="ad-muted">{formatearFecha(c.fecha)}</td>
                      </tr>
                    ))}
                  </tbody>
                </table>
        ...
        {seccion === 'repuestos' && (
          <div className="ad-section">
            <div className="ad-page-header">
              ...
              <button className="ad-btn-main" onClick={() => setModalPrecio(true)}>
                ✏️ Editar precio
              </button>
            </div>
                  ...
                  <tbody>
                    {repuestos.map(r => (
                      <tr key={r.id}>
                        ...
                        <td>
                          <strong style={{ color: '#e2ecf8' }}>{r.nombre}</strong>
                        </td>
            ...
            <div className="ad-page-header">
              <div>
                ...
                <p className="ad-page-sub">
                  {tecnicos.length} técnicos registrados · {tecnicos.filter(t => t.disponible).length}{' '}
                  disponibles ahora.
                </p>
              </div>

              <button className="ad-btn-main" onClick={() => setModalTecnico(true)}>
                + Nuevo técnico
              </button>
            </div>
                  ...
                  <tbody>
                    {tecnicos.map(t => {
                      ...
                      const ordHoy = servicios.filter(
                        s => String(s.tecnicoId) === String(t.id) && s.fecha === hoy
                      ).length;
          ...
          <div className="ad-section">
            <div className="ad-page-header">
              <div>
                ...
                <p className="ad-page-sub">
                  {sols.length} solicitud{sols.length !== 1 ? 'es' : ''} recibida
                  {sols.length !== 1 ? 's' : ''} desde el portal cliente.
                </p>
              </div>
            </div>
              ...
              <div className="ad-table-wrap">
                <table className="ad-table">
                  ...
                  <tbody>
                    {sols.length === 0 ? (
                      <tr>
                        <td colSpan={6} className="ad-empty-row">
      ...
      {modalTecnico && (
        <Modal
          ...
          footer={
            <>
              <button className="btn btn-secondary" onClick={() => setModalTecnico(false)}>
                Cancelar
              </button>
        ...
        >
          ...
          <div className="form-group">
            ...
            <input
              value={tecCorreo}
              onChange={e => setTecCorreo(e.target.value)}
              placeholder="tecnico@refrimora.com"
            />
          </div>
      ...
      {modalPrecio && (
        <Modal
          ...
          footer={
            <>
              <button className="btn btn-secondary" onClick={() => setModalPrecio(false)}>
                Cancelar
              </button>
          ...
          <div className="form-row">
            <div className="form-group">
              ...
              <input
                value={
                  epRep
                    ? formatearPeso(repuestos.find(r => r.id === parseInt(epRep))?.precio || 0)
                    : ''
                }
                readOnly
            ...
            <div className="form-group">
              ...
              <input
                type="number"
                value={epNuevo}
                onChange={e => setEpNuevo(e.target.value)}
                min={0}
              />
            </div>
          </div>
          ...
          servicios.map(sv => {
            ...
            return (
              <tr key={sv.id}>
                ...
                <td>
                  <div className="ad-td-user">
                    <AvatarCliente nombre={cl?.nombre} />
                  </div>
                </td>
                <td className="ad-muted">{cl?.direccion || '—'}</td>
                <td>{tc?.nombre || '—'}</td>
                <td>{sv.tipo}</td>
